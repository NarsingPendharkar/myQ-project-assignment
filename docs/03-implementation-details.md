...
- pom.xml: Spring Boot 3.3.1 LTS, Java 21, all dependencies
- AuditLogServiceApplication.java: Entry point with @EnableScheduling
- Constants.java: Application-wide constants
- application.properties: PostgreSQL, JWT, Flyway, retention policies
- application-dev.properties: Development profile with H2
...
- Updated V1 migration to work with both PostgreSQL (JSONB) and H2 (CLOB)
- Uses BIGINT GENERATED BY DEFAULT AS IDENTITY (compatible with both)
- Uses CLOB for JSON storage (compatible with both databases)
